...
export interface GameState {
   ...
   * the next seat's turn (`product.mdc` feedback).
   */
...
export interface LegalMove {
  ...
  /** Opponent pieces sent home when landing on a non-safe track square. */
  readonly captures: readonly PieceId[];
}
...
export function rollDice(state: GameState, die: DieValue): GameState {
  ...
  const cleared: GameState = { ...state, lastRoll: null };

  if (die === 6 && cleared.consecutiveSixes === 2) {
    // Third six in a row → turn ends, no move on this roll.
    return { ...passTurn(cleared), lastRoll: die };
  }

  const nextSixes = die === 6 ? cleared.consecutiveSixes + 1 : cleared.consecutiveSixes;
...
function tryComputeMove(state: GameState, piece: PieceId, die: DieValue): LegalMove | null {
  ...
  // Yard: only a 6 can move a piece onto the start square (stacking allowed).
  if (from.zone === 'yard') {
    if (die !== 6) return null;
    const start = getStartTrackIndex(color);
    const to: PiecePosition = { zone: 'track', index: start };
    const captures = findCaptureVictims(state, color, to);
    return { piece, to, captures };
  }
...
function findCaptureVictims(
  state: GameState,
  attacker: PlayerColor,
  to: PiecePosition,
): readonly PieceId[] {
  if (to.zone !== 'track') return [];
  if (isSafeTrackIndex(to.index)) return [];
  const victims: PieceId[] = [];
...
/** Update runtime control for one seat (online disconnect / reconnect). */
export function withSeatKind(
  state: GameState,
  seatIndex: number,
  kind: PlayerKind,
): GameState {
  if (seatIndex < 0 || seatIndex >= state.seatKinds.length) {
    throw new RangeError(`seatIndex out of range: ${seatIndex}`);
  }
  const seatKinds = [...state.seatKinds];
  seatKinds[seatIndex] = kind;
  return { ...state, seatKinds };
}
...
export function applyMove(state: GameState, piece: PieceId): GameState {
  ...
  for (const victim of chosen.captures) {
    positions[pieceKey(victim)] = { zone: 'yard' };
  }
